...
class Request {
  ...
  //请求配置
  requestConfig(headers, timeout, baseURL, withCredentials) {
    switch (this.type) {
      case 'fly':
      ...
      if (this.type === 'axios') {
        ...
        this.request
          .request({
            method: type,
            url: url,
            data: this.qs ? this.qs.stringify(params) : params
          })
          .then(response => {
            resolve(response)
          })
      ...
      if (this.type === 'axios') {
        arg = {
          ...
          paramsSerializer: params => {
            return this.qs ? this.qs.stringify(params) : params
          }
        }
      } else {
        arg = this.qs ? this.qs.stringify(params) : params
      }
